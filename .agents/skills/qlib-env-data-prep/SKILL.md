...
### 脚本治理
- `scripts/audit_skill_scripts.py`
- `scripts/cleanup_temp_scripts.py`
...
2. 再执行可用性检查与端到端验证。
3. 执行时序覆盖门禁（MFA 默认切分）：
   - 训练窗口起点需覆盖 `2000-01-04`
   - OOS 需覆盖 `2025-01-01` 起的可用区间
   - 若 2026 数据未完整，必须在输出中标注“截断 OOS”
4. 若失败，优先修复环境再交给下游 skill。
5. 需要 workflow 数据治理时，执行 schema 迁移与文档回填。
6. 在交付前执行脚本治理审计与清理。

## 脚本治理守门

1. 检查 `.agents/skills/*/scripts/` 是否混入一次性脚本。
2. 检查并清理 skills 内 `__pycache__` / `.pyc`。
3. 一次性脚本统一放 `./scripts/`，完成后删除。
4. 保留证据产物（`outputs/`、`docs/`、DB记录），不保留一次性脚本本体。

## 证据要求

- 输出目录：`outputs/`（检查结果、脚本日志、审计报告）
- 关键结论应可回溯到具体脚本输出或 DB 记录。
- 数据门禁结论必须明确写出覆盖终点日期（例如 `2026-02-13`）。
