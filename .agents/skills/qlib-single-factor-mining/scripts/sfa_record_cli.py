...
DEFAULT_DOCS_DIR = PROJECT_ROOT / "docs" / "workflows" / "single-factor"
...
def _upsert_from_args(db: WorkflowDB, args: argparse.Namespace) -> None:
    ...
    if any(
        ...
        for name in ["rank_ic_mean", "rank_ic_t", "fdr_p", "rank_icir", "n_days", "sfa_result"]
    ):
        db.upsert_sfa_metrics(
            round_id=args.round_id,
            rank_ic_mean=args.rank_ic_mean,
            rank_ic_t=args.rank_ic_t,
            fdr_p=args.fdr_p,
            rank_icir=args.rank_icir,
            n_days=args.n_days,
            sfa_result=_val(args.sfa_result),
        )
...
def cmd_create(db: WorkflowDB, args: argparse.Namespace) -> None:
    ...
    print(f"[OK] Created/updated SFA round: {args.round_id}")


def cmd_update(db: WorkflowDB, args: argparse.Namespace) -> None:
    _upsert_from_args(db, args)
    print(f"[OK] Updated SFA round: {args.round_id}")


def cmd_list(db: WorkflowDB, args: argparse.Namespace) -> None:
    rows = db.list_runs(round_type="sfa", market=args.market, decision=args.decision, limit=args.top)
    if not rows:
        print("No SFA rounds found.")
        return
    for row in rows:
        compact = {
...
def cmd_sync_index(db: WorkflowDB, args: argparse.Namespace) -> None:
    rows = db.list_runs(round_type="sfa", limit=args.top)
    ...
    for row in rows:
        lines.append(
            "| {round_id} | {date} | {hypothesis_short} | {market} | {rank_icir} | {sfa_result} | {decision} | {doc_path} |".format(
                ...
                rank_icir=row.get("rank_icir") if row.get("rank_icir") is not None else "N/A",
                sfa_result=row.get("sfa_result") or "N/A",
...
def _add_common_round_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--rank-ic-mean", type=float)
    parser.add_argument("--rank-ic-t", type=float)
    parser.add_argument("--fdr-p", type=float)
    parser.add_argument("--rank-icir", type=float)
    parser.add_argument("--n-days", type=int)
    parser.add_argument("--sfa-result")
...
def main() -> int:
    ...
    p_sync = sub.add_parser("sync-index", help="Sync SFA index markdown")
